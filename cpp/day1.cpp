...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "ERROR - Invalid number of arguments: expected 1, got "
              << (argc - 1) << '\n';
    return -1;
  }
  ...
  const auto diffs =
      std::views::zip_transform(subtractAbs, leftVector, rightVector);
  ...
  auto countOccurance = [&](auto acc, auto &elem) {
    const auto counts =
        std::count(rightVector.cbegin(), rightVector.cend(), elem);
    return acc + (elem * counts);
  };

  const int result2 = std::accumulate(leftVector.cbegin(), leftVector.cend(), 0,
                                      countOccurance);
